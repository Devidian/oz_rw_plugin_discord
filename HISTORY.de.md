...
- fix: `restart` command hat falschen Text im Spiel angezeigt

## [0.15.5] - 2021-05-16

- fix: der neue `command` parameter hat die klassen-Eigenschaft `command` überschrieben

## [0.15.4] - 2021-05-15

- fix: discord Befehle haben als ersten parameter den Befehl selbst übergeben.

## [0.15.3] - 2021-05-04

- refactor: Bot Befehle in eigene Klassen separiert mit Hilfe von `sdcf4j`
- refactor: Discord message Listener wurde in eigene Datei verschoben und reagiert nur noch auf chat

## [0.15.2] - 2021-05-03

- fix: Abhängigkeiten aktualisiert
...
- feat: Der Bot sollte jetzt auf ein Kommando eine reaction abgeben
- feat: Aufzeichnen von `badass` Aktionen über eine neue webhook url
  - Melde Tötungen von Reittieren
  - Melde Tötungen von nicht aggresiven Tieren
  - Melde entfernen von Werkbänken etc.
- feat: Einstellungen für den `Badass Tracker`
...
- feat: Öffentliche API für Status Meldungen
- feat: Geplante Server neustart(s)
- feat: Einstellungen für `Geplante Neustarts`
...
- feat: Neue Discord `Informelle` Befehle
...
- feat: Neue Discord `Administrative` Befehle
...
- fix: Exception bei Eingabe von `/dp` ohne parameter
- fix: JavaCordBot entfernt jetzt alle listener bei einem disconnect
- fix: Der Reguläre Ausdruck für `noColorText` kannte keine Zahlen
- refactor: Alle \*.md files sind nun standardmäßig in englisch
- refactor: Abfragebefehle fangen jetzt mit `?` statt `!` an. Siehe `?help` in Discord oder README
- chore: Maven Abhängigkeiten (JavaCord 3.0.1 -> 3.0.4)

## [0.14.0] - 2019-03-20

- feat: neue Farbeinstellung `colorLocalAdmin` für Admin-Nachrichten (Player.isAdmin)
- feat: neue Einstellung `colorizeChat` um die manipulation des lokalen chat zu deaktivieren
- feat: neue Einstellung `showGroup` um die permission group hinter den Spielernamen anzuzeigen
- refactor: Alle farbwerte können jetzt in `settings.properties` eingestellt werden

## [0.13.3] - 2019-03-18

- feat: einige Listener zum JavaCordBot hinzugefügt um Verbindungsabbrüche zu loggen.
- feat: neue Einstellung in den `settings.properties` => `supportScreenshot` um automatischen screenshot bei support nachrichten einzustellen
- fix: Fehler der den Absturz vor 0.13.2 verursacht hatte wurde behoben

## [0.13.2] - 2019-03-18

- fix: Der Fehler der einen Absturz bei einem Plugin Update verursachte, sollte nun abgefangen werden.
- fix: Fehlende deutsche Übersetzung wurde hinzugefügt

## [0.13.1] - 2019-03-15

- fix: der support webhook war fälschlicherweise der chat-webhook

## [0.13.0] - 2019-03-15

- fix: verwendet nun die tools.jar in Version 0.4.1
- fix: textfarben wurden aus `DiscordWebHook` Klasse entfernt, Colors aus tools wird jetzt verwendet
- fix: MOTD entfernt, heisst jetzt Plugin Welcome Message und kann nur noch ein und aus geschaltet werden, definition des textes ist jetzt in den Sprachdateien, damit jeder Spieler die Nachricht in seiner Sprache lesen kann (sofern übersetzt wurde)
- fix: "Plugin geladen" Meldung wird jetzt in onEnabled gefeuert
- fix: in initSettings wird jetzt "Einstellungen geladen" in das serverlog geschrieben

- feat: wenn `+screen` in eine Nachricht geschrieben wird, wird ein screenshot übermittelt der in Discord angezeigt werden kann (nicht ingame)
- feat: in den `settings.properties` kann eingestellt werden ob screenshots erlaubt sind und welche maximale Auflösung übertragen werden soll (angabe der breite)
- feat: bei support nachrichten wird immer ein screenshot gesendet (kann sich in Zukunft vielleicht noch ändern)

## [0.12.0] - 2019-01-30

- feat: Befehle die mit `/dp` beginnen hinzugefügt
- feat: neuer Befehl `/dp info` zeigt die Beschreibung des Plugins
- feat: neuer Befehl `/dp help` zeigt alle möglichen Befehle des Plugins
- feat: neuer Befehl `/dp status` zeigt den aktuellen Status des Plugins wie Version und Sprache

- fix: die log methode in `DiscordWebHook` wurde durch den Logger aus `tools` ersetzt
- fix: `JavaCordBot` hat nun seinen eigenen Logger aus `tools`
- fix: broadcast Nachrichten werden nun anhand der Spieler-Systemsprache übersetzt
- fix: Befehls-Nachrichten werden nun auch anhand der Spieler-Systemsprache übersetzt
- fix: `/ozrestart` ist jetzt deprecated und wird bald entfernt, verwende `/dp restart` statt dessen
- fix: Alle Nachrichten haben nun ein Plugin-prefix

## [0.11.5] - 2019-01-27

- refactor: Die plugin-übergreifende `tools.jar` muss nun unter `/plugins/shared/lib/tools.jar` zur Verfügung stehen

## [0.11.4] - 2019-01-26

- chore: tools.jar auf 0.2.0 aktualisiert
