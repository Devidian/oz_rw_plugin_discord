...
public class SetHealthCommand implements CommandExecutor {
    ...
    static final String command = "sethealth";
    static final String fullCommand = "/" + command;
    static final String description = "tbd";

    @Command(aliases = {
            fullCommand }, description = description, async = true, privateMessages = false, usage = "/sethealth [playerName] [value=100]", showInHelpPage = true)
    public String onCommand(TextChannel channel, Message message, MessageAuthor author, String cmd,
        ...
        if (playerName == null) {
            return t.get("CMD_ERR_ARGUMENT_LENGTH", lang).replace("PH_CMD", "/sethealth [PLAYER] [VALUE=100]");
        }
        if (valueInt == null) {
            valueInt = "100";
        }
